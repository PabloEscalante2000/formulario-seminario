...
<div class="flex items-center justify-center min-h-screen px-4 py-12">
    <div class="w-full max-w-sm">
        <div class="bg-white rounded-2xl shadow-xl p-8">
            <h1 class="text-2xl font-bold text-brand-black text-center mb-6">Panel de Administración</h1>

            @error('email')
                <div class="bg-brand-brown/10 text-brand-brown text-sm rounded-lg p-3 mb-4">{{ $message }}</div>
            @enderror
            ...
            <form method="POST" action="{{ route('admin.login') }}" class="space-y-5">
                ...
                <div>
                    <label for="email" class="block text-sm font-medium text-neutral-700 mb-1">Correo</label>
